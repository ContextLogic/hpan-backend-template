"""
ratelimit defines the global singleton of ratelimit client class.
...
class RatelimitClient:
    ...
    _url: str = "http://ratelimit-stage.service.consul:8080"
    ...
    def ratelimit(cls, ratelimit_name: str) -> bool:
        ...
        Return True if got a 429 status code, otherwise False.
        Return False on exception like timeout etc.
        """
    ...
    def increase_hit(cls, ratelimit_name: str) -> None:
        """
        Used for increase the ratelimit count on alki(https://alki.i.wish.com/) side.
        This is to improve the ratelimiting accuracy.
        This function is called when a task is about to run.
        """
        if not ratelimit_name:
            # if ratelimit name is not given, we will use the default ratelimit name under
            # python-backend-worker-template domain
