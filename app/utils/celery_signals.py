"""
utils.celery_signals defines the signal handlers,
...
from app.utils.metrics import (
    ...
    TASKS_RUNTIME_SECONDS,
    TASKS_LATENCY_SECONDS,
)
from app.utils.ratelimit import RatelimitClient
from app.utils.tracing import new_tracer

logger = logging.getLogger(__name__)

"""
Below defines all the celery task signal handlers, for more info about celery task signal, please refer to
...
def connect_signal_handlers() -> None:
    ...
    worker_process_shutdown.connect(worker_process_shutdown_handler, weak=False)
...
def disconnect_signal_handlers() -> None:
    ...
    worker_process_shutdown.disconnect(worker_process_shutdown_handler, weak=False)
...
def worker_process_init_handler(*args: object, **kwargs: object) -> None:
    """
    when worker proecss initialized, we create a new tracer and install celery tracing patches for it.
    """
    tracer = new_tracer()
...
def worker_process_shutdown_handler(*args: object, **kwargs: object) -> None:
    """
    when worker process shut down, we close the tracer.
    """
    global_tracer().close()


def task_prerun_handler(
    task_id: str = None, task: Task = None, args: object = None, **kwargs: object
) -> None:
    """
    ...
    if received_timestamp:
        ...
        TASKS_LATENCY_SECONDS.labels(name=task_name, queue=queue).observe(
            latency_time.total_seconds()
        )
    ...
    RatelimitClient.increase_hit(QUEUES.ratelimit_name(queue))
...
def task_postrun_handler(
    ...
    **kwargs: object
    ...
    if prerun_timestamp:
        ...
        TASKS_RUNTIME_SECONDS.labels(name=task_name, queue=queue).observe(
...
def task_failure_handler(task_id: str = None, **kwargs: object) -> None:
    """
...
def task_success_handler(**kwargs: object) -> None:
    """
    handler function gets run when a task execution is successful. It mainly used for emitting metrics
    """
...
def task_internal_error_handler(**kwargs: object) -> None:
    """
...
def task_received_handler(request: Request = None, **kwargs: object) -> None:
    """
...
def task_rejected_handler(message: Message = None, **kwargs: object) -> None:
    """
    handler function gets run when a worker receives an unknown type of message to one of its task queues.
    It mainly used for emitting metrics
    """
...
def task_revoked_handler(
    ...
    **kwargs: object
    ...
    WORKER_TASKS_COUNT.labels(
        name=format_map["task_name"], queue=format_map["queue"], state="REVOKED"
    ).inc()
    logger.debug(
        "Task {task_name}[{task_id] from {queue} gets revoked, terminated: {terminated}, signum: {signum}, "
        "expired: {expired}".format_map(format_map)
    )


def task_unknown_handler(
    name: str = None, message: Message = None, **kwargs: object
) -> None:
    """
    handler function gets run when a worker receives a message for a task that’s not registered.
    It mainly used for emitting metrics
    """
    delivery_info = safe_extract(message, "delivery_info", {})
    queue = delivery_info.get("routing_key")
