...
# This task is an alternative processing logic in dlq in case task retry_demo fails after retries
@current_app.task(
    name="err_retry_demo",  # task name, should be matching with the Queue definition in app.config.queues
    bind=True,
    base=RetryBaseTask,  # inherited the RetryBaseTask, so this task can be auto retried.
)
def err_retry_demo(self: Task, num: int) -> float:
    """
    a retry demo function called by retry_demo task, used for demoing celery auto retry on some specific exceptions.
    """
    time.sleep(2)
