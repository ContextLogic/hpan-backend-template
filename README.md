...
### Tracing/Jaeger
We automatically setup/configure the global opentracing (deprecated) / opentelemetry.
...
## Work with Mongodb
When you work with a new microservice or move some services out of clroot, you should use the new Mongoproxy Native. Meanwhile, you can have the flexibility of using latest version of mongoengine instead of [the old one](https://github.com/ContextLogic/mongoengine). In this template, we provide an example usage of Mongoproxy Native + mongoengine (/examples/mongo/) for the purpose of demonstration. In our case, we use pymongo==3.11.0 to avoid issues discussed [here](https://logicians.slack.com/archives/C0133DZ2WE8/p1628702176095800) and mongoengine==0.24.0. Any new features or breaking changes of mongoengine are introduced [here](https://github.com/MongoEngine/mongoengine/blob/master/docs/changelog.rst). If you wish to use some newer version of mongoengine, update it in pyproject.toml and run `poetry update` to pick up the changes.

You can find the access points in this [wiki doc](https://wiki.wish.site/display/Infra/Micro-services+Mongo+Cluster). 

Once you feel confident about your services and they are ready for production, you need to work with infra storage team first, i.e. create a new database (plan for indexing/sharding), check in your schemas. Those steps are described in the [wiki](https://wiki.wish.site/display/Infra/Micro-services+Mongo+Cluster) as well.

Here's a great wiki page about some best practices by i18n team: [link](https://wiki.wish.site/pages/viewpage.action?pageId=69670452)
